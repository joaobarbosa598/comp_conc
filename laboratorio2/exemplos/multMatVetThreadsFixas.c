...
int main(int argc, char* argv[]){
	...
	if(argc<3){ //argc sempre vai ter tamanho 1 ou mais, o primeiro parametro é o nome do arquivo
		printf("Digite: %s <dimensao da matriz> <número de threads>\n", argv[0]);
		return 1;
	}
	...
	args = (tArgs *) malloc(sizeof(tArgs)*nThreads); //o dim aqui seria o numero de linhas e não coluna
...
void * multiplicaMatrizVetor(void * arg){
	...
	for(int i=args->id; i < args->dim; i+=nThreads){
		for(int j=0; j<args->dim; j++){
				saida[i] += matriz[i*(args->dim) +j] * vetor[j];
		}
	}
	pthread_exit(NULL);
}